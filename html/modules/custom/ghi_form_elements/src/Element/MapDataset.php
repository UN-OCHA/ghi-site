...
class MapDataset extends FormElementBase {
  ...
  public static function getInitialDatasetItem(array $element, array $parents, bool $required = FALSE): array {
    $attachment_id = reset($element['#attachment_ids']);
    $default_values = [
      'attachment' => $attachment_id,
      'metric' => $required ? self::getFirstMetricType($attachment_id) : self::NONE,
      'settings' => [],
    ];
  ...
  private static function getUsedFieldsFromDataset(array $dataset, int $attachment_id): array {
    ...
    foreach ($items as $item) {
      ...
      if ($item['metric'] == self::NONE || $item['metric'] === NULL) {
        continue;
      }
  ...
  public static function processMapDataset(array &$element, FormStateInterface $form_state) {
    ...
    if (!$form_state->has('field_context')) {
      ...
      foreach ($attachment_ids as $attachment_id) {
        ...
        // Only use planning metrics and measurement metrics, but not calculated
        // fields as these are not mappable.
        $fields = $attachment->getPlanningFields() + $attachment->getMeasurementFields();
        $metric_availability = self::getMetricAvailability($attachment);
        $available_fields = $metric_availability === NULL ? array_keys($fields) : array_merge($metric_availability['base'], ...array_values($metric_availability['measurements']));
        $form_state->set(['field_context', $attachment_id], [
          'fields' => $fields,
          'used_fields' => [],
          'empty_fields' => array_values(array_diff(array_keys($fields), $available_fields)),
    ...
    foreach ($dataset['slices'] as $i => $slice) {
      if ($i > 0 && $dataset['slices'][$i - 1]['metric'] == self::NONE) {
        continue;
      }
      $rows['slices'][$i] = [
      ...
      for ($j = $i + 1; $j < $max_slices; $j++) {
        if (!empty($dataset['slices'][$j]) && $dataset['slices'][$j]['metric'] != self::NONE) {
  ...
  private static function buildDatasetRow($element, FormStateInterface $form_state, $row) {
    ...
    $current_metric = $row['dataset']['metric'] ?? ($required ? self::getFirstMetricType($attachment_id) : self::NONE);
    ...
    if (!$settings_edit) {
      ...
      if (!empty($row['disabled_metrics'])) {
        $build['metric']['#options'] = array_diff_key($build['metric']['#options'], array_flip($row['disabled_metrics']));
      }

      // Keep unavailable and already used metrics visible so editors can see
      // the complete set of choices. The current selection remains enabled so
      // its saved value is still submitted until it is replaced.
      $disabled_options = array_values(array_intersect(
        array_unique(array_merge(
          array_diff($field_context[$attachment_id]['used_fields'] ?? [], [$current_metric]),
          array_diff($field_context[$attachment_id]['empty_fields'] ?? [], [$current_metric]),
        )),
        array_keys($build['metric']['#options']),
      ));
      if ($disabled_options) {
        $build['metric']['#attributes']['data-map-dataset-disabled-options'] = Json::encode($disabled_options);
      }
      ...
      if ($current_metric != self::NONE) {
        $summary_items[] = (string) t('Label: @label', [
          '@label' => ($settings['label'] ?? NULL) ?: self::getMetricLabel($attachment, $current_metric),
        ]);
      }

      if ($attachment?->isMeasurementField($current_metric)) {
      ...
      $metric_availability = $field_context[$attachment_id]['metric_availability'] ?? NULL;
      $availability_warning = $attachment && $current_metric != self::NONE && $metric_availability !== NULL
        ? MapMetricAvailabilityHelper::getWarning($attachment, (string) $current_metric, $settings, $metric_availability, self::getMetricLabel($attachment, $current_metric), $required)
        : NULL;
      if ($summary || $availability_warning) {
        $build['settings_summary'] = [
          '#type' => 'container',
          '#wrapper_attributes' => [
            'class' => [
        ...
        if ($summary) {
          $build['settings_summary']['summary'] = [
            '#markup' => Markup::create('<div class="summary">' . $summary . '</div>'),
          ];
        }
        if ($availability_warning) {
          $build['settings_summary']['availability_warning'] = [
            '#type' => 'container',
            '#attributes' => [
              'class' => ['dataset-availability-warning'],
            ],
            '#markup' => $availability_warning,
          ];
        }
      }
    ...
    if ($settings_edit) {
      ...
      $build['settings']['context_summary'] = [
        ...
        '#markup' => new TranslatableMarkup('<strong>@attachment</strong>: @metric', [
          ...
          '@metric' => self::getMetricLabel($attachment, $current_metric) ?? t('No metric selected'),
        ]),
      ];
      $build['settings']['label'] = [
        ...
        '#attributes' => [
          'placeholder' => $current_metric != self::NONE ? (string) self::getMetricLabel($attachment, $current_metric) : '',
        ],
      ];
      $build['settings']['monitoring_period'] = [
        ...
        '#plan_id' => $plan_id,
        '#add_wrapper' => FALSE,
        '#access' => $attachment?->isMeasurementField($current_metric),
      ];
  ...
  private static function getSelectOptions(int $attachment_id, int|string $current_metric, array $field_context, $required = FALSE) {
    ...
    foreach ($fields as $key => $label) {
      $options[$key] = in_array($key, $empty_fields, TRUE) && !in_array($key, $used_fields, TRUE)
        ? (string) $label . ' — ' . t('No location-level data')
        : $label;
    }
    return $options;
  }
   ...
   */
  public static function loadAttachment($attachment_id): ?Attachment {
    $query = self::getAttachmentQuery();
    $attachment = $query->getAttachment((int) $attachment_id);
    return $attachment instanceof Attachment ? $attachment : NULL;
  }

  /**
   * Get the attachment query plugin.
   */
  private static function getAttachmentQuery(): AttachmentQuery {
    return \Drupal::service('plugin.manager.fabric_query_manager')->createInstance('attachment');
  }

  /**
   * Get mappable metric availability for an attachment.
   */
  private static function getMetricAvailability(Attachment $attachment): ?array {
    return self::getAttachmentQuery()->getMappableMapMetricAvailability($attachment);
  }
  ...
  /**
   * Get the first mappable metric type for an attachment.
   */
  private static function getFirstMetricType($attachment_id): int|string {
    $attachment = self::loadAttachment($attachment_id);
    if (!$attachment) {
      return self::NONE;
    }
    $fields = $attachment->getPlanningFields() + $attachment->getMeasurementFields();
    return array_key_first($fields) ?? self::NONE;
  }

  /**
   * Get a metric label from an attachment.
   */
  private static function getMetricLabel(?Attachment $attachment, int|string|null $metric): ?string {
    if (!$attachment || $metric === NULL || $metric == self::NONE) {
      return NULL;
    }

(function ($) {
  ...
  window.ghi.compositeMap = class {
    ...
    buildFullPieFeatures = function () {
      let full_pie = this.state.getData().full_pie;
      let locations = this.getFullPieLocations();
      let features = locations.map(object => object.feature ?? this.buildFeatureForObject(object, full_pie));
    ...
    filterEmptyLocations = function (locations) {
      let full_pie = this.state.getData().full_pie;
      return locations.filter((object) => object.metrics[full_pie.attachment.id][full_pie.metric_index] > 0);
    }
    ...
    updateFeatures = function (duration = null) {
      ...
      let state = this.state;
      let full_pie = this.state.getData().full_pie;
      let features = state.updateFeatures(this.sourceId, this.featureLayerId, (object) => object.feature ?? self.buildFeatureForObject(object, full_pie));
      return features;
    }
    ...
    createAdminAreaOutline = function (object, border_color) {
      ...
      let geojson = this.map.getGeoJSON(object, null, false);
      if (!geojson) {
        return;
      }
      const width = 160
    ...
    */
     ...
     */
    buildFeatureForObject = function (object, full_pie) {
      ...
      return {
        ...
        'properties': {
          ...
          'total': object.metrics[full_pie.attachment.id][full_pie.metric_index],
    ...
    addAdminAreaLayers = function () {
      ...
      if (this.showPolygons()) {
        ...
        color_steps.shift();
        Object.assign(paint, {
          'fill-color': color_steps.length > 1 ? [
            'step',
            ['get', 'value'],
          ].concat(color_steps) : color_steps[0],
          'fill-opacity': 1,
        });
      }
      else {
      ...
      for (let item of legend_items) {
        ...
        if (item.toggle_class) {
          ...
          $legend_item.on('click', function () {
            ...
            state.getCanvasContainer()
              .find('.donut svg path.' + item.toggle_class)
              .toggle(!$(this).hasClass('inactive'));
          });
        }
    ...
    buildSidebarContent = function(object) {
      ...
      if (typeof location.modal_contents == 'undefined') {
        let lazy_modal_contents = state.getModalContent(location);
        if (!lazy_modal_contents) {
          return;
        }
        location.modal_contents = lazy_modal_contents;
      }
    ...
    buildSidebarContentSection = function (data, type) {
      ...
      return `
        <div class="section" data-section-type="${type}">
          <div class="header">
            <span>
              <span class="table-toggle material-icon" tabindex="0" aria-hidden="true">
                <svg aria-hidden="true" focusable="false">
                  <use href="#material-symbol-keyboard-arrow-down" xlink:href="#material-symbol-keyboard-arrow-down"></use>
                </svg>
              </span>
              ${data.metric_label}
            </span>
    ...
    handleFeatureClick = function (e = null, feature = null) {
      ...
      this.state.showSidebarForObject(object);
    }
    ...
    getDataRanges = function() {
      let polygon_data = this.getPolygonData();
      let total = this.getPolygonLocations(true, false).map(d => d.metrics[polygon_data.attachment.id][polygon_data.metric_index]).filter(Number);
      ...
      if (tooltip === null) {
        ...
        for (const [i, slice] of Object.entries(this.getSliceData())) {
          ...
          tooltip_items.push({
            ...
            color: this.config.colors['slices'][i],
          });
        }
        ...
        for (let tooltip_item of tooltip_items) {
          let swatch = tooltip_item.color ? '<span class="map-tooltip-list__swatch" style="background-color: ' + tooltip_item.color + '"></span>' : '';
          tooltip += '<div class="map-tooltip-list__row">' + swatch + '<span class="map-tooltip-list__text"><b>' + tooltip_item.label + ':</b> ' + tooltip_item.value + '</span></div>';
        }
      }
      return '<div class="map-tooltip-list">' + tooltip + '</div>';
    }

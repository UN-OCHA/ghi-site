 ...
 */
class SyncElementsManager implements ContainerInjectionInterface {
   ...
   *
   * @return array
   *   An array with result messages for the node.
   *
   * @throws SyncElementsException
   *   When an error occurs.
   */
  public function syncNode(NodeInterface $node) {
    $settings = $this->config->get('ghi_plans.settings');
    $messages = [];
    ...
    if (empty($settings->get('sync_source'))) {
      throw new SyncElementsException('Error: Source is not configured');
    }
    ...
    if ($code != 200) {
      throw new SyncElementsException('Error: Invalid response');
    }

    $body = $response->getBody()->getContents();
    if (empty($body)) {
      throw new SyncElementsException('Error: Empty response');
    }

    $data = json_decode($body);
    if (empty($data->status)) {
      throw new SyncElementsException('Error: Access key misconfigured or object not valid');
    }
    ...
    foreach ($data->elements as $element) {
      ...
      if ($existing_component) {
        ...
        $messages[] = $this->t('Updated %plugin_title', [

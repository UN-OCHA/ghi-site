...
def read_timings(directory):
    """Sum test cases (including data sets) without counting parent suites twice."""
    timings = {}
    for report in sorted(directory.rglob("*.xml")):
        try:
            root = ET.parse(report).getroot()
        except (ET.ParseError, OSError) as error:
            print(f"Ignoring unreadable timing report {report}: {error}")
            continue
        for case in root.iter("testcase"):
            # PHPUnit records absolute container paths, not checkout paths.
            path = case.get("file", "")

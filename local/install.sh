...
if [ "$create_image" = "yes" ]; then
  ...
  make REGISTRY=$REGISTRY IMAGE_NAME=$IMAGE_NAME IMAGE_TAG=$IMAGE_TAG
fi;
...
if [ "$install_dev_dependencies" = "yes" ]; then
  ...
  echo "Ensure the downloads directory exists and is writable."
  docker_compose exec site mkdir -p /srv/www/html/sites/default/files/downloads
  docker_compose exec site chmod 777 /srv/www/html/sites/default/files/downloads
